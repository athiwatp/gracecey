...
// DefaultTimeout of timeout for sleep
...
// FlushFinish implements for graceful shutdown of servers.
type FlushFinish struct {
...
func New() *FlushFinish {
	return &FlushFinish{}
}

func (f *FlushFinish) signals() []os.Signal {
	if f.Signals != nil {
		return f.Signals
	}
...
func (f *FlushFinish) timeout() time.Duration {
	if f.Timeout != 0 {
		return f.Timeout
	}
...
func (f *FlushFinish) getManSig() chan interface{} {
	f.mutex.Lock()
...
func (f *FlushFinish) Add(srv Server, opts ...Option) {
	keeper := &serverKeeper{
		srv:     srv,
		timeout: f.timeout(),
	}
...
func (f *FlushFinish) Trigger() {
	f.getManSig() <- nil
}

func (f *FlushFinish) updateNames() {
